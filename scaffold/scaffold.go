...
// package consts. Called before every render so a scaffolded agent always pins
// the versions this scaffold owns — callers never pass them.
func (d *ScaffoldData) applyToolchainVersions() {
	if d.GoVersion == "" {
		d.GoVersion = GoVersion
	}
	if d.TemplVersion == "" {
		d.TemplVersion = TemplVersion
	}
	if d.TailwindVersion == "" {
		d.TailwindVersion = TailwindVersion
	}
	if d.DaisyUIVersion == "" {
...
var templateFiles = []templateFile{
	...
	{"templates/icons.templ.tmpl", "views/icons.templ"},
	...
	// the MVC split notes in agentsdk/REFERENCE.md.
	...
	{"templates/handlers_home_test.go.tmpl", "handlers/home_test.go"},
	...
	// reference (/libs/agentsdk/REFERENCE.md) is the orthogonal half: this
	// file says how to wire things together, REFERENCE.md says what the SDK
	// gives you.
	{"templates/AGENTS.md.tmpl", "AGENTS.md"},
	// NOTES.md is agent-owned and carries stable headings for current product,
	// architecture, integration, and UI decisions.
	{"templates/NOTES.md.tmpl", "NOTES.md"},
...
var staticFiles = []templateFile{
	...
	// deps, Tailwind/DaisyUI CSS, and the Lucide icon catalog).
	// Generated by agentsdk/scripts/gen-notices.sh; required by those licenses to travel
...
func Materialize(dir string, data ScaffoldData) error {
	...
	tmpl, err := template.New("").
		Funcs(template.FuncMap{"generatedArtifactIgnorePatterns": GeneratedArtifactIgnorePatterns}).
		Option("missingkey=error").
