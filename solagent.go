...
func newSolAgent(a *Agent, run *run, supportedModalities []string) *agent.Agent {
	...
	return &agent.Agent{
		...
		SystemPrompt:      a.renderSystemPrompt(run.callerAccess, run.visibleSiblings), // rendered per-run from live registrations + synced PromptData; mutex-guarded so /refresh updates are visible
